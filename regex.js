...
function markDownLink (text) {
    if((/[^!]\w*\.[\w]{2,3}[\/\)]/).test(text)&&!(/^!/).test(text)){
        var urlRegex = /\(([\w:\/.]*)\)/
        var url = urlRegex.exec(text)[1]
        var labelRegex = /\[([\w:\/. ]*)\]/
        var label = labelRegex.exec(text)[1]

        link = text.replace(/\[[\w:\/. ]*\]/,"")
        link = link.replace(/\(/,'<a href=\"')
        link = link.replace(/\)/,'\">'+label+'</a>')
...
function markDownItalics (text) {
    toBeItalic = text.match(/\*{1}([\w. ]*)\*/g)

    if(toBeItalic !== null){
        for(i=0 ; i <toBeItalic.length ; i++){
            if(toBeItalic[i] !== '**'){
                text = text.replace(toBeItalic[i], '<em>'+toBeItalic[i].slice(1, -1)+'</em>')
            }
        }
    }
    return text 
